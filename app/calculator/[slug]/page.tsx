...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: tool.title || `${tool.name} – CalcCent`,
    description: tool.metaDescription || `Free ${tool.name} – calculate ${tool.name.toLowerCase()} instantly.`,
  };
}
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8 border border-gray-100 my-6">
        {/* ✅ Pass the slug AND a key to force re-render when slug changes */}
        <UniversalCalculator 
          key={slug}
          initialMode={slug} 
        />
      ...
      <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 mt-8">
        ...
        <p className="text-gray-700 bg-white p-4 rounded-lg border border-gray-100 font-mono text-sm">{tool.formula}</p>
        ...
        {schema && (
          <div className="mt-6 pt-4 border-t border-gray-200">
            <p className="text-sm text-gray-500">
              <span className="font-medium">Input Type:</span> {schema.inputType}
            </p>
          </div>
        )}
