...
export default function Home() {
  ...
  const getIcon = (category: string) => {
    const icons: Record<string, string> = {
      'Shopping': '🛍️', 'Finance': '💰', 'Math': '📐', 'Tax': '🧾',
      'Business': '📊', 'Education': '📚', 'Health': '💪',
      'Real Estate': '🏠', 'Life': '⏰', 'Cooking': '🍳'
    };
  ...
  return (
    <div>
      ...
      <section className="relative z-20 max-w-4xl mx-auto px-4 -mt-8 md:-mt-12 pb-8 md:pb-12">
        ...
        <div className="mt-6"><AdUnit /></div>
      </section>
      <section id="tools" className="max-w-6xl mx-auto px-4 py-12 md:py-20">
        <div className="text-center mb-10 md:mb-14">
          <h2 className="text-2xl md:text-4xl font-extrabold text-gray-800">All <span className="gradient-text">50 Calculators</span></h2>
          <p className="text-gray-500 mt-3 max-w-xl mx-auto text-sm md:text-base">Explore our complete collection of percentage and financial calculators</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
          {tools.map((tool, index) => (
            <Link key={tool.slug} href={`/calculator/${tool.slug}`} className="tool-card rounded-2xl p-4 text-center transition-all hover:scale-[1.02] hover:shadow-lg">
      ...
      <section className="bg-gradient-to-br from-indigo-50 to-purple-50 py-12 md:py-20 border-t border-gray-200">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <h2 className="text-2xl md:text-3xl font-extrabold text-gray-800 mb-3">Trusted by Thousands</h2>
          <p className="text-gray-600 max-w-2xl mx-auto text-sm md:text-base">Join millions of students, business owners, and finance enthusiasts who use CalcCent daily.</p>
